...
var.corr.test <- function(data, base.taxa, shift.taxa, r = NULL, n.perm, max.attempts = 10 * n.perm){
  if(is.null(r)){
  ...
  n_reduced <- numeric(n.perm)
  nstar_reduced <- numeric(n.perm)
  ...
  while(perm < n.perm){
    ...
    n_reduced[perm] <- sum(table(pp_original_reduced$marks)[c(base.taxa, shift.taxa)])
    ...
    pp_prob <- pp_prob/sum(pp_prob)
    nstar_reduced[perm] <- (pp_prob[base.taxa]*pp_prob[shift.taxa])
  }
  ...
  mean.reduced <- rowMeans(K_perm, na.rm = TRUE)
  S0 <- (K0 - mean.reduced)*sqrt(lambda1)*sqrt(lambda2)
  values.simulated <- Kstar_perm
  values.std <- sweep(values.simulated, 1, mean.reduced, "-")
  values.std <- sweep(values.std, 2, sqrt(nstar_reduced), "*")
  cset_std <- curve_set(obs = S0, sim = values.std, r = r)
  plot(cset_std)
  obj <- global_envelope_test(cset_std, typeone = "fwer", alternative = "greater")
  pval <- attr(obj, "p")
